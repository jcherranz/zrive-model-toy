...
| 13 | 2026-08-16 | the salt was printed in the header of the file it protects, on a public repository | the register is untracked and comes from a repository secret, the salt is random and lives in one, and a register that cannot match aborts every gate |
...
## The salt was printed in the header of the file it protects, and the repository had gone public

`2026-08-16-salt-published-in-the-file-it-protects` &middot; 2026-08-16

**What happened.** `scripts/forbidden_names.sha256` is the register the three name gates match
against. It holds no names, only salted truncated hashes, and that was the whole of its defence.
Its third line read, in clear, in the file itself: the number of real people covered, the number
of tokens, and **the salt**. The same salt was assigned in `scripts/forbidden_lib.sh`. Both files
were tracked, and `gh api ... --jq .visibility` returns `public`.

**Why the existing concession did not cover it.** The generator's own header said, correctly, that
hashing bought obscurity and not secrecy, because the salt was committed and Spanish given names
and surnames are a short dictionary. That paragraph was written for a **private** repository, and
three documents still said the repository was private: two lines in README.md, one in TPS.md, two
in HANSEI.md, five in total. The concession outlived its premise and read as a risk somebody had
priced, when nobody had re-priced it after the visibility changed.

**The measurement.** The exact construction, single-threaded, in pure Python, on an ordinary
laptop: a little over eight hundred thousand hashes a second. A hundred thousand candidate tokens
against the whole register is roughly a tenth of a second. Two auditors reached the same order
independently. It is not a hard problem and it never was; what changed is who was allowed to try.

**What was actually wrong with the reporting, which is the part worth keeping.** This was measured
twice before and understated twice. The first attempt used plain SHA-256 and matched nothing,
because the gate folds and salts. The second used the project's own folding and hashing, confirmed
a handful of tokens from a short word list, and was reported as a narrow membership oracle over
common surnames. **Neither reading noticed that the salt and the headcount were sitting in the
file's own header**, and a decision to leave the file alone was taken on that reporting. A partial
measurement that answers the question asked is not the same as a reading of the artifact, and the
artifact here was three lines long.

**What was done.** The register is untracked and ignored; it is generated by
`scripts/gen_forbidden_hashes.sh` on a machine that holds the vault, and CI, which holds none,
gets the same bytes from a repository secret through `scripts/ci_register.sh`, written outside the
working tree. The salt is random, lives in a secret, and `scripts/forbidden_lib.sh` refuses to
load without it. The header carries no headcount and no salt; it carries a salt-check that binds
the register to the salt that built it, so a register that cannot match aborts every gate instead
of scanning happily and reporting clean. The five document lines are corrected.

**Prevention kind:** `structural` and `gate`
**Named by:** `scripts/forbidden_lib.sh`, `scripts/gen_forbidden_hashes.sh`,
`scripts/ci_register.sh`, `scripts/check_repo.sh`, `scripts/check_forbidden.sh`, `build/model.py`

**Open, and stated rather than closed quietly.** Rotation stops the file being a live oracle from
today and reaches backwards not at all: the register as it stood, and the salt it was built under,
are in this repository's history and are public. Removing them is a history rewrite, which is the
decision recorded under `2026-08-09-gate-read-the-disk-not-the-repository` and is the owner's; that
entry declined the rewrite partly because the repository was private, and that half of its reasoning
is gone. **Whether the
repository stays public is also his**, and is not decided here: making it private again would
return Actions to the billing block that stopped every workflow for eleven hours, and it would not
undo a disclosure already made.
