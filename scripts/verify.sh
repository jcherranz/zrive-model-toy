...
# now have a complete checkout and be unable to run six of the thirteen steps below, which is a
# state this file did not previously have to describe.
#
...
# read it, and a run that lost six of thirteen steps and printed a summary would be inviting a
# reader to weigh a number that means nothing. The whole run is refused instead, before the
# banner, with the reason and the remedy. That keeps this file at thirteen steps and keeps every
# one of them a step that ran.
#
...
preflight_register() {
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    printf '%s\n' "$out"
    echo
    echo "Six of the thirteen steps below read the name register and none of them can run"
    echo "without it. A run that skipped them would print a ratio that means nothing, so"
    echo "there is no run."
    exit 2
  fi
  ...
  if [ ! -s "$hashes" ]; then
    ...
    echo "  and must never be published. Six of the thirteen steps below read it: the build"
    echo "  gate and its self-test, the two provenance gates, both repository gate steps and"
    echo "  the forbidden-content gate. Every one of them recognises a name by hashing it and"
